function run_optim_model_3(country)
...
[param_s, hessian_s] = optim_model_3(theta_s, data_de_spot, data_de_spot(:, 7));
std_s = sqrt(diag(inv(hessian_s)));

out_s = [param_s, std_s];

theta_i = ones(20, 1) * 0.2;
theta_i = ones(21, 1) * 0.2;
...
[param_i, hessian_i] = optim_model_3(theta_i, data_de_intraday, data_de_spot(:, 7));
std_i = sqrt(diag(inv(hessian_i)));

out_i = [param_i, std_i];
csvwrite(sprintf('C:/git/r/powerfor/inst/matlab/log_lik_model_3/output/fit_model_3_%s_spot.csv', country), out_s);
csvwrite(sprintf('C:/git/r/powerfor/inst/matlab/log_lik_model_3/output/fit_model_3_%s_intraday.csv', country), out_i);
